...
type Options = {
  ...
  extraSubscriptions?: Record<string, any>[];
  ...
  private connect() {
    ...
    try {
      ...
      ws.on("open", () => {
        ...
        for (const m of [...subscriptions(this.options.venue, this.options.ids, this.options.includeTrades, this.options.includePolyTrades),...(this.options.extraSubscriptions??[])]) {
