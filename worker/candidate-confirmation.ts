...
import {executableFingerprint,RouteObservation} from '../lib/screen/discovery.ts';
...
export function confirmationManifest(root:string){return {...sourceManifest(root),'scripts/confirmation-launch.mjs':sha(readFileSync(resolve(root,'scripts/confirmation-launch.mjs'))),'docs/research/aoc-contract-review-20260921.json':sha(readFileSync(resolve(root,'docs/research/aoc-contract-review-20260921.json')))};}
...
export async function prepareConfirmation(dir:string,baseline:string,root:string,control=false){
 ...
 save(dir,'frozen.json',{at,mode:control?'CONTROL':'EXPERIMENT',policy,manifest:confirmationManifest(root),selection,subscriptions,baselineFreezeSha256:sha(readFileSync(resolve(baseline,'frozen.json'))),baseline:'September 21 17:13–17:43 UTC completed screen; 2.63% was strict freshness pass rate, not uptime'});
...
export type BatchLimits={durationMs:number;maxAttempts:number;maxEvidenceBytes:number;preparedAt:number};
export async function runConfirmation(dir:string,root:string,batch?:BatchLimits){
 if(batch&&(!Number.isSafeInteger(batch.durationMs)||batch.durationMs<1||batch.durationMs>integratedPolicy.durationMs||!Number.isSafeInteger(batch.maxAttempts)||batch.maxAttempts<0||batch.maxAttempts>integratedPolicy.maxAttempts||!Number.isSafeInteger(batch.maxEvidenceBytes)||batch.maxEvidenceBytes<1||batch.maxEvidenceBytes>integratedPolicy.maxEvidenceBytes||Date.now()-batch.preparedAt>integratedPolicy.durationMs+600000))throw Error('Invalid discovery batch limits');
 const frozen=JSON.parse(readFileSync(resolve(dir,'frozen.json'),'utf8')),control=frozen.mode==='CONTROL',policy=batch?{...integratedPolicy,durationMs:batch.durationMs,maxAttempts:batch.maxAttempts,maxEvidenceBytes:batch.maxEvidenceBytes}:policyFor(control);
 if(!!batch!==(frozen.mode==='DISCOVERY_BATCH'))throw Error('Discovery batch requires its single-window supervisor');
 if(existsSync(resolve(dir,'started.json')))throw Error('Single-use capture; no restart');
 if(JSON.stringify(frozen.policy)!==JSON.stringify(policy)||JSON.stringify(frozen.manifest)!==JSON.stringify(confirmationManifest(root)))throw Error('Frozen source or policy changed');
 if((!batch&&Date.now()-frozen.at>600000)||!frozen.selection.length)throw Error('Preparation expired or empty');
 ...
 const observation=new RouteObservation(frozen.selection.map((e:any)=>e.pair),startWall),requested=new Set<string>();
 const best=new Map<string,any>(),bestUsable=new Map<string,any>(),latest=new Map<string,any>(),lastAttempt=new Map<string,{at:number;fingerprint:string}>();
 const attemptedRoutes=new Set<string>(),attemptedFingerprints=new Set<string>();
 ...
 const streamRecord:RecordEvidence=(kind,body)=>{if(kind==='WS_SUBSCRIPTION_REQUEST'){const m=(body as any).message;for(const id of m.params?.market_tickers??[])requested.add('kalshi:'+id);for(const id of m.subscribe?.marketSlugs??[])requested.add('poly:'+id);}if(kind!=='WS_BOOK')record(kind,body);if(kind==='WS_SUBSCRIPTION_REQUEST'&&(body as any).message?.params?.channels?.includes('market_lifecycle_v2'))statusTracker.requested((body as any).at);};
 ...
 const evaluate=(id:string)=>{const entry=entries.get(id),pair:Pair=entry.pair,a=feeds.kalshi.latest.get(pair.a.id)?.e,b=feeds.poly.latest.get(pair.b.id)?.e,at=Date.now(),mono=performance.now();
  ...
  for(const side of ['yes','no'] as Side[]){const key=pair.id+'|'+side,old=intervals.active.get(key),q=quoteCandidate(pair,a?.book,b?.book,side,at);evaluations++;
   ...
   if(interval&&interval!==old)record('CANDIDATE_INTERVAL_OPEN',{...interval,row});if(old&&interval!==old)record('CANDIDATE_INTERVAL_CLOSE',intervals.completed.at(-1));
   if(q.economics&&(!best.has(key)||q.economics.feeBoundSurplus>best.get(key).economics.feeBoundSurplus))best.set(key,row);
   if(row.dataValidity.usableForDiscovery&&q.economics&&(!bestUsable.has(key)||q.economics.feeBoundSurplus>bestUsable.get(key).economics.feeBoundSurplus))bestUsable.set(key,row);
  }
 };
 ...
 const confirm=async(pair:Pair,original:ReturnType<typeof quoteCandidate>,intervalId:number|null,key:string)=>{
  busy=true;activeConfirmation={intervalId,original,controlOnly:control};attempts++;lastRequestMono=performance.now();lastAttempt.set(key,{at:lastRequestMono,fingerprint:executableFingerprint(original)});attemptedRoutes.add(pair.id);attemptedFingerprints.add(executableFingerprint(original));
 ...
 const summary=(endedAt?:number)=>({scope:'ORDER_DISABLED_INTEGRATED_CONFIRMATION',mode:frozen.mode,phase:endedAt?'STOPPED':'RUNNING',startedAt:startWall,endedAt:endedAt??null,durationMs:(endedAt??Date.now())-startWall,
  ...
  routeObservation:[...observation.rows.values()],bestUsable:[...bestUsable.values()],bestDisplayed:[...best.values()],latestClassification:[...latest.values()],seenBooks:seen.size,expectedBooks:frozen.subscriptions.kalshi.length+frozen.subscriptions.poly.length,
 ...
 else await bootstrapStatus(statusTracker,frozen.subscriptions.kalshi,async id=>{if(stopped)throw Error('STOPPED');return publicConfirmationGet('https://external-api.kalshi.com/trade-api/v2/markets/'+encodeURIComponent(id),record);},policy.statusSpacingMs,()=>stopped);
 if(stopped){process.off('SIGINT',stop);process.off('SIGTERM',stop);return;}
 ...
 work=setInterval(()=>{if(stopped)return;if(resourceStop){stop();return;}if(!clockOkay()){resourceStop='CLOCK_FAULT';stop();return;}
  ...
  for(const [id,entry]of entries){const pair:Pair=entry.pair,a=feeds.kalshi.latest.get(pair.a.id)?.e,b=feeds.poly.latest.get(pair.b.id)?.e,at=Date.now(),mono=performance.now(),av=validity(a,health('kalshi'),at,mono),bv=validity(b,health('poly'),at,mono);observation.sample(id,at,requested.has('kalshi:'+pair.a.id)&&requested.has('poly:'+pair.b.id),!!a&&!!b,av.usableForDiscovery&&bv.usableForDiscovery,[...av.reasons.map(r=>'KALSHI_'+r),...bv.reasons.map(r=>'PM_US_'+r)],['yes','no'].some(side=>!!latest.get(id+'|'+side)?.economics));}
  ...
  const candidates=[...intervals.active.values()].filter(x=>{const last=lastAttempt.get(x.key),pair:Pair=entries.get(x.original.pairId).pair;return !status(pair.a.id).knownBlocked&&!attemptedFingerprints.has(executableFingerprint(x.latest))&&(!last||performance.now()-last.at>=policy.perCandidateCooldownMs);}).sort((a,b)=>Number(attemptedRoutes.has(a.original.pairId))-Number(attemptedRoutes.has(b.original.pairId))||Number(entries.get(a.original.pairId).contractReview.referenceCase===true)-Number(entries.get(b.original.pairId).contractReview.referenceCase===true)||(b.latest.economics?.feeBoundSurplus??0)-(a.latest.economics?.feeBoundSurplus??0)||a.id-b.id);
  const next=candidates[0];if(next)void confirm(entries.get(next.original.pairId).pair,next.latest,next.id,next.key);
 },policy.coalesceMs);
 ...
 await completion;process.off('SIGINT',stop);process.off('SIGTERM',stop);
 // Drain the single in-flight request before the next bounded batch starts.
