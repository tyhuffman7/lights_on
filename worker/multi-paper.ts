...
import {MultiPaper,multiPolicy as policy,reviewedRows,bookFingerprint} from '../lib/arb/multi-paper.ts';
...
export async function prepare(dir:string,root:string){
  ...
  save(dir,'frozen.json',{at:Date.now(),policy,manifest:manifest(root),metadata,blocked,documents,review,settlementRiskSource:'docs/research/contract-shortlist/REVIEW.md sections Q/R/T; no equivalence or cash-release guarantee'});
}
...
export async function observe(dir:string,root:string){
  ...
  const portfolio=new MultiPaper(frozen.review),metadata=frozen.metadata as Metadata[],blocked=new Map<string,string>(frozen.blocked.map((x:any)=>[x.pairId,x.reason]));
  ...
  const persist=()=>{save(dir,'journal.json',{portfolio:portfolio.summary(),entries:portfolio.entries});save(dir,'summary.json',{scope:policy.mode,phase:stopped?'STOPPED':ready?'RUNNING':'REBUILDING_BOOKS',startedAt,endedAt:stopped?Date.now():null,deadlineAt:startedAt+policy.durationMs,reason,policy,ordersEnabled:false,liveAdmitted:false,retainedComparisons:34,refreshedComparisons:metadata.length,blocked:[...blocked].map(([pairId,reason])=>({pairId,reason})),observations:[...observations.values()],rejectionCounts:rejections,qualifyingCandidates:[...qualifying],automaticCandidates:[...automatic],confirmationRequests:portfolio.confirmations,confirmations,arrivals,feedEvents,accounting:portfolio.summary(),modeled:portfolio.entries.map(({row,session:s})=>({pairId:row.pairId,plans:s.plan,legs:s.results,recovery:s.recovery})),rawEvidenceBytes:bytes,settlement:'CONDITIONAL; no settlement modeled or guaranteed cash-release date; unrealized surplus is not profit',synthetic:false});};
  const heartbeat=setInterval(()=>{persist();console.log(JSON.stringify({mode:'live-data',scope:policy.mode,phase:ready?'RUNNING':'REBUILDING_BOOKS',elapsedMs:performance.now()-startMono,attempts:portfolio.entries.length,confirmations:confirmations.length,observed:observations.size,ordersEnabled:false}));},10000);
  ...
  const request=async(m:Metadata)=>{
    await sleep(nextRequest-performance.now());if(stopped||performance.now()>=deadline)throw Error('REQUEST_DEADLINE');
    nextRequest=performance.now()+policy.requestSpacingMs;
  ...
  const arrival=(m:Metadata,v:Venue,proofs:Proofs,due:number)=>{
  ...
  try{
    ...
    while(!stopped&&performance.now()<deadline){
      try{await connect();break;}catch(e){feedEvents.push({at:Date.now(),reason:(e as Error).message});for(const f of Object.values(feeds!))f.stop();if(!portfolio.reconnect(performance.now(),deadline,2))throw e;await sleep(1000);}
    }
    while(!stopped&&performance.now()<deadline){
      if(portfolio.stopReason()){reason=portfolio.stopReason()!;break;}
      if(!healthy()){
        ...
        if(!portfolio.reconnect(performance.now(),deadline,affected.length)){reason='RECONNECTION_LIMIT_OR_DEADLINE';break;}
      ...
      for(const id of ids){
        ...
        if(portfolio.confirmations>=policy.maxConfirmations){count(['CONFIRMATION_REQUEST_CAP']);continue;}
        if(dataReasons.length||!comparison.selected)continue;
        let proofs:Proofs;
        ...
        try{
          const result=await portfolio.execute(m.row,m.pair,original,async()=>{
            ...
            persist();record('FROZEN_SUBMISSION',{pairId:id,plans:s.plan});
            await Promise.all((['kalshi','poly'] as const).map(async v=>{const plan=s.plan![v];await sleep(plan.arrivalMono-performance.now());const ev=arrival(m,v,proofs,plan.arrivalMono),result=modelLeg(plan,v==='kalshi'?m.pair.a:m.pair.b,ev.book,ev.reasons,v==='kalshi'&&status(m).knownBlocked,ev.wall,plan.arrivalMono);accountLeg(s,m.pair,v,result);record('MODELED_LEG',{pairId:id,venue:v,result});persist();}));
            if(recoveryPlan(s,performance.now())&&!stopped&&performance.now()+2500<deadline){
              await sleep(policy.recoveryDelayMs);
              try{const rpProofs=await request(m),rp=recoveryPlan(s,performance.now());if(rp&&rp.arrivalMono<deadline){record('FROZEN_RECOVERY',{pairId:id,plan:rp});await sleep(rp.arrivalMono-performance.now());const ev=arrival(m,rp.venue,rpProofs,rp.arrivalMono);recover(s,rp,rp.venue==='kalshi'?m.pair.a:m.pair.b,ev.book,ev.reasons,ev.wall,rp.arrivalMono);}}
          ...
          if(result.attempted)automatic.add(id);count(result.reasons);record('AUTOMATIC_ENTRY_DECISION',{pairId:id,result});persist();
        }catch(e){const error=(e as Error).message;count([error]);confirmations.push({pairId:id,at:Date.now(),error});record('CONFIRMATION_OR_EXECUTION_ERROR',{pairId:id,error});}
      }
    }
    reason=portfolio.stopReason()??reason;
    ...
    console.log(JSON.stringify({mode:'live-data',message:'MULTI_PAPER_STOPPED',reason,accounting:portfolio.summary(),ordersEnabled:false}));
  }
}
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){const [mode,output,env]=process.argv.slice(2);if(!output||!['prepare','observe'].includes(mode))throw Error('Usage: multi-paper.ts prepare|observe DIRECTORY [ENV_FILE]');if(env)process.loadEnvFile(env);if(mode==='prepare')await prepare(resolve(output),process.cwd());else await observe(resolve(output),process.cwd());}
