...
import {SegmentedEvidence} from './segmented-evidence.ts';
...
export async function runStudy(dir:string,root:string,originalDeadline:number){
 ...
 const evidence=new SegmentedEvidence(resolve(dir,'evidence'));
 ...
 const rows=new Map(entries.map(e=>[e.pair.id,{pairId:e.pair.id,category:e.category,kalshiId:e.pair.a.id,polyId:e.pair.b.id,classification:e.contractReview.status,
  ...
  recordedExtrema:null as ReturnType<typeof derivedQuote>,largestQuantity:null as ReturnType<typeof derivedQuote>,previousAt:startMono,previousUsable:false,previousReasons:['INITIALIZING'],previousQuotes:[] as StudyQuote[],stateSignature:''}]));
 ...
 const record:RecordEvidence=(kind,body)=>{if(stopped&&!['CONFIRMATION_RESULT','CONFIRMATION_ERROR'].includes(kind))return;try{evidence.append(kind,body,busy||!['WS_BOOK','DERIVED_CHANGE'].includes(kind));bytes=evidence.totalBytes;}catch{stop('EVIDENCE_RESOURCE_OR_WRITE_FAILURE');throw Error('EVIDENCE_RESOURCE_OR_WRITE_FAILURE');}};
 ...
 const evaluate=(e:typeof entries[number],changed=false)=>{if(stopped)return;const s=state(e),row=rows.get(e.pair.id)!;accrue(row,s.mono,s.usable);evaluations++;
  ...
  if(changed)for(const q of row.previousQuotes.map(derivedQuote)){if(q&&(!row.recordedExtrema||q.feeNetGapPerContract>row.recordedExtrema.feeNetGapPerContract))row.recordedExtrema=q;if(q&&(!row.largestQuantity||q.quantity>row.largestQuantity.quantity||(q.quantity===row.largestQuantity.quantity&&q.feeNetSurplus>row.largestQuantity.feeNetSurplus)))row.largestQuantity=q;}
 ...
 const summary=()=>({scope:'ORDER_DISABLED_READ_ONLY_STREAMING_DISPERSION',phase:stopped?'STOPPED':'RUNNING',startedAt:start,deadlineAt:originalDeadline,endedAt:stoppedAt,reason,ordersEnabled:false,ledgerAccess:false,policy,
  evidenceStorage:evidence.snapshot(),evaluations,attempts,clockFault,bytes,reconnections:rebuild.used,feedEvents:events,feeds:Object.fromEntries((['kalshi','poly'] as Venue[]).map(v=>[v,feeds[v]?{health:health(v),transport:feeds[v].stream.health(),failures:feeds[v].failures,receivedMarkets:feeds[v].latest.size,expectedMarkets:ids[v].length}:null])),
 ...
 heartbeat=setInterval(()=>{try{evidence.checkpoint();}catch{stop('EVIDENCE_RESOURCE_OR_WRITE_FAILURE');return;}save(dir,'summary.json',summary());console.log(JSON.stringify({mode:'live-data',phase:'RUNNING',elapsedMs:Date.now()-start,attempts,intervals:intervals.rows.length,reconnections:rebuild.used,received:[feeds.kalshi?.latest.size??0,feeds.poly?.latest.size??0],ordersEnabled:false}));},policy.heartbeatMs);
 let rebuilding=true;
 const tick=async()=>{if(stopped)return;if(!clockOkay()){stop('CLOCK_FAULT');return;}if(Date.now()>=originalDeadline){stop('ORIGINAL_DEADLINE');return;}
  ...
  for(const [id,p]of pending){const interval=intervals.rows.find(r=>r.id===id)!;if(!intervals.active.has('fee|'+interval.key))continue;
   ...
   record('CONFIRMATION_START',{intervalId:id,original:derivedQuote(q),pair:p.entry.pair,review:p.entry.contractReview,books:{kalshi:s.a,poly:s.b},health:{kalshi:health('kalshi'),poly:health('poly')}});
   try{const result:any=await confirmScreenCandidate(p.entry.pair,q,feeds,()=>status(p.entry.pair.a.id),p.entry.contractReview,record);
    ...
    confirmations.push(row);record('CONFIRMATION_RESULT',{row,completeRequestedProof:result});
   }catch{record('CONFIRMATION_ERROR',{intervalId:id});confirmations.push({intervalId:id,startedAt:began,endedAt:Date.now(),status:'FAILED',reasons:['CONFIRMATION_ERROR'],original:derivedQuote(q),bookConfirmed:false,fillClaim:false});}
   finally{busy=false;}break;
  }
 };
 healthTimer=setInterval(()=>{void tick().catch(()=>stop('STUDY_PROCESSING_FAILURE'));},policy.healthSampleMs);
 try{await connect(['kalshi','poly']);rebuilding=false;await completion;while(busy)await sleep(20);evidence.close();save(dir,'summary.json',summary());}
