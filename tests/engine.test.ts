...
test('Sports are paper eligible but settlement approval remains required',()=>{assert.ok(assess({...pair,reviewed:false},a,b,defaults,cash,now)?.reasons.includes('Settlement rules need review'));assert.ok(assess({...pair,a:{...pair.a,category:'Sports'}},a,b,defaults,cash,now)?.eligible)});
