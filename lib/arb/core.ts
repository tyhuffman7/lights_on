...
import {fee, integer, GENERAL_KALSHI_RATE, PM_US_SEPTEMBER_2026_RATE} from '../research/fees.ts';
...
export function calculate(a: number, b: number, q: number, reserve = 100) {
  ...
  const fees=fee(q,a,GENERAL_KALSHI_RATE,'ceil')+fee(q,b,PM_US_SEPTEMBER_2026_RATE,'even');
  const cost=(a+b)*q, payout=q*USD, buffer=reserve*q, profit=payout-cost-fees-buffer;
  return {cost,fees,reserve:buffer,payout,profit,roi:profit/(cost+fees+buffer)*100,feeVersion:'PM-US-2026-09-17',feeRates:{kalshi:GENERAL_KALSHI_RATE,poly:PM_US_SEPTEMBER_2026_RATE},feeScope:'HYPOTHETICAL_ESTIMATE_NO_REBATES'};
}
