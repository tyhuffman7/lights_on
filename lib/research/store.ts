...
const tables=new Set(['sessions','mappings','mapping_history','book_updates','opportunities','opportunity_states','latency_tests','diagnostics']);
export class ResearchStore{
 db:DatabaseSync;path:string;
 constructor(path:string){
  this.path=path;mkdirSync(dirname(path),{recursive:true});this.db=new DatabaseSync(path);
 ...
 rows(table:string){if(!tables.has(table))throw new Error('Unknown table');return this.db.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all() as Record<string,any>[];}
 transaction<T>(fn:()=>T):T{this.db.exec('BEGIN IMMEDIATE');try{const x=fn();this.db.exec('COMMIT');return x;}catch(e){this.db.exec('ROLLBACK');throw e;}}
 diagnostic(session:string,at:number,kind:string,body:unknown){this.db.prepare('INSERT INTO diagnostics(session_id,at,kind,body) VALUES(?,?,?,?)').run(session,at,kind,JSON.stringify(body));}
 close(){this.db.close();}
}
