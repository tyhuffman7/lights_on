...
function px(v: unknown) {
  const s = String(v);
  if (!/^(?:0?\.\d{1,4}|0|1(?:\.0{1,4})?)$/.test(s))
    throw new Error("Invalid price precision");
  const p = Math.round(Number(s) * USD);
  if (p <= 0 || p >= USD) throw new Error("Invalid book price");
  return p;
}
...
function canonical(ls: Level[], bid = false) {
  ...
  for (const l of ls) {
    if (map.has(l.price)) throw new Error("Duplicate price");
    if (l.quantity > 0) map.set(l.price, l.quantity);
  }
  return [...map]
    .map(([price, quantity]) => ({ price, quantity }))
    .sort((a, b) => (bid ? b.price - a.price : a.price - b.price));
}
function complement(ls: Level[]) {
  return ls.map((l) => ({ ...l, price: USD - l.price }));
}
export class BookCache {
  ...
  get(venue: Venue, id: string) {
    return this.books.get(`${venue}:${id}`);
  }
  reset(venue: Venue) {
  ...
  private save(
    ...
    yesBids = canonical(yesBids, true);
    noBids = canonical(noBids, true);
    const b: StreamBook = {
      venue,
      marketId: id,
      yes: canonical(complement(noBids)),
      no: canonical(complement(yesBids)),
      yesBids,
      noBids,
      receivedAt: at,
      receivedMono: mono,
    ...
    if (b.yes[0] && yesBids[0] && yesBids[0].price >= b.yes[0].price)
      throw new Error("Crossed or locked book");
    this.books.set(`${venue}:${id}`, b);
    return b;
  }
  kalshi(
    ...
    if (!["orderbook_snapshot", "orderbook_delta"].includes(data.type))
      return null;
    try {
      ...
      if (
        ...
        throw new Error("Missing sequence/market");
      const prev = this.sequences.get(sid);
      if (prev !== undefined && seq !== prev + 1)
        throw new Error("Kalshi sequence gap or reorder");
      this.sequences.set(sid, seq);
      const id = String(m.market_ticker);
      ...
      if (data.type === "orderbook_snapshot") {
        ...
        no = structuredClone(old.noBids);
        if (m.side !== "yes" && m.side !== "no")
          throw new Error("Unknown side");
        const ls = m.side === "yes" ? yes : no,
          p = px(m.price_dollars),
  ...
  poly(data: Record<string, any>, at: number, mono: number): StreamBook | null {
    ...
    try {
      ...
      if (!m.marketSlug || !Array.isArray(m.bids) || !Array.isArray(m.offers))
        throw new Error("Missing full market book");
      const parse = (ls: Record<string, any>[]) =>
        ls.map((l) => {
          if (l.px?.currency !== "USD") throw new Error("Non-USD book");
          return { price: px(l.px.value), quantity: quantity(l.qty) };
        });
      ...
      return this.save(
        ...
        String(m.marketSlug),
        parse(m.bids),
        complement(parse(m.offers)),
        at,
        mono,
        ts,
        null,
        m.state === "MARKET_STATE_OPEN",
      );
    } catch (e) {
      this.invalidate("poly");
      throw e;
    }
  }
}
export function fresh(
  ...
  return (
    ...
    b.source !== "rest" &&
    mono >= b.receivedMono &&
    mono - b.receivedMono <= maxAge &&
    Math.abs(wall - b.receivedAt) <= maxAge &&
    (b.exchangeAt === null ||
      (wall >= b.exchangeAt - 1000 && wall - b.exchangeAt <= maxAge))
  );
}
