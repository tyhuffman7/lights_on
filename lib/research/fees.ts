...
export function feeSchedule(m: Market): FeeSchedule | null {
  if (
    m.feeRate === null ||
    !Number.isSafeInteger(m.feeRate) ||
    m.feeRate < 0 ||
    m.feeRate > 10000
  )
    return null;
  if (m.venue === "kalshi" && m.feeRounding === "ceil")
    return {
      rate: m.feeRate,
      rounding: "ceil",
      aggregation: "level",
  ...
  if (m.venue === "poly" && m.feeRounding === "even")
    return {
      ...
      aggregation: "order",
      source:
        "PM-US market coefficient; rounded cumulative taker fee upper bound",
    };
  return null;
}
export function feesForLevels(
  levels: Level[],
  schedule: FeeSchedule | null,
): number {
  if (!schedule) throw new Error("Unknown fee schedule");
  if (schedule.aggregation === "order")
    return rounded(
      levels.reduce(
        (n, l) => n + numerator(l.quantity, l.price, schedule.rate),
        0n,
      ),
      schedule.rounding,
    );
  return levels.reduce(
    (n, l) => n + fee(l.quantity, l.price, schedule.rate, schedule.rounding),
    0,
  );
}
...
export function feeBounds(levels: Level[], schedule: FeeSchedule) {
  ...
  const upper =
    schedule.aggregation === "order"
      ? estimated
      : levels.reduce(
          (n, l) =>
            n + l.quantity * fee(1, l.price, schedule.rate, schedule.rounding),
          0,
        );
