...
if(!output||!envFile)throw Error('Usage: executable-screen-launch.mjs OUTPUT_DIRECTORY ENV_FILE');
...
const result=await supervise({command:process.execPath,args:['--experimental-strip-types',resolve(root,'worker/executable-screen.ts'),'observe',dir,resolve(envFile)],cwd:root,log:resolve(dir,'worker.log'),status:resolve(dir,'status.json'),durationMs:1800000,silenceMs:60000,graceMs:5000,forceMs:5000,minFreeBytes:2*1024**3});
awake?.kill('SIGTERM');
