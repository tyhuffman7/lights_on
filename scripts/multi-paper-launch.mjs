...
if(!output||!envFile)throw Error('Usage: multi-paper-launch.mjs DIRECTORY ENV_FILE');
...
const result=await supervise({command:process.execPath,args:['--experimental-strip-types',resolve(root,'worker/multi-paper.ts'),'observe',dir,resolve(envFile)],cwd:root,log:resolve(dir,'worker.log'),status:resolve(dir,'status.json'),durationMs:3600000,silenceMs:60000,graceMs:5000,forceMs:5000,minFreeBytes:2*1024**3});
